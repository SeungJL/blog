...
title: 올린 것보다 올리지 않은 것을 더 오래 셌다
excerpt: 2년 6개월 동안 만드는 데 시간을 다 썼으니 스택도 그때 그대로였다. 런타임부터 한 칸씩 올리기로 하고 순서를 정했다.
category: 기술
tags:
  - Next.js
  - Node.js
...
만드는 데 시간을 다 썼다는 건 쓰는 도구를 손볼 시간은 따로 두지 않았다는 뜻이기도 했다. `package.json`을 열었더니 이렇게 적혀 있었다.
...
순서를 정했다. 에디터, 런타임, 프레임워크, 도구. **위에서부터 올리면 아래가 못 받쳐서 되돌아온다.**

<!-- ME: VSCode를 올리면서 실제로 겪은 것이 있으면 여기에 적어주세요. 두세 문장이면 충분하고, 다듬지 않아도 됩니다. 없으면 이 자리는 지웁니다. -->

런타임을 올릴 때 Next를 한 번에 15로 보내지 않았다. 14.2.16에서 **14.2.35로 먼저 옮겼다.** Next 14.2를 Node 21 이상에서 구동할 때 보고된 런타임 이슈가 있어서, 14.2 계열의 최신 패치로 발을 딛고 나서 런타임을 바꿨다. 두 개를 동시에 흔들면 뭐가 깨졌는지 모르게 된다.
...
| `images.domains` | `localhost:3000` 포함 | `domains`는 포트를 받지 않는다. 게다가 `images.unoptimized: true`라 이 설정군 자체가 실효가 없다 |
...
React 19를 올린다는 건 저 세 줄을 함께 하겠다는 결정이다. 단독으로 올릴 수 있는 항목이 아니다.
...
`node_modules`를 지우고 `npm ci`로 다시 깐 상태에서, `tsc --noEmit` → `next build`(정적 페이지 156개) → `next start`. 여덟 개 라우트가 전부 200을 주는지 확인했다. `middleware`의 쿠키 분기, styled-components의 SSR 스타일 추출도 각각 확인했다.
...
여기서 사람이 한 일은 이 목록을 만든 것이다. 무엇을 확인해야 하는지 알고 있었기 때문에 나머지를 맡길 수 있었다. 검증 항목을 못 세우면 통과했다는 말에 아무 의미가 없다.
...
Chromatic에는 Storybook 8과 9의 렌더링 차이가 그대로 남아 있다. 아직 안 봤다.

## 다음 순서

이번에 옮긴 건 런타임(Node 24.20.0), 프레임워크(Next 15.5.25), 도구(Storybook 9.1.20)다. 앱 코드는 거의 건드리지 않았다.
